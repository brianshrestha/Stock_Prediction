...
def display_explanation(input_df, session, aws_bucket):
    ...
    feature_names = [f"Component_{i}" for i in range(input_df_transformed.shape[1])]
...
if submitted:
    processed_input = convert_input_pca_regression(
        json.dumps(user_inputs),
        "application/json"
    )

    payload = processed_input.to_dict(orient="records")

    res, status = call_model_api(payload)
    if status == 200:
